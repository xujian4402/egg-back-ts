...
module.exports = (options, app) => {
  return async function token_parser(ctx, next) {
    ...
    const token_id = app.config.wxappApi.token_id;

    const api_regexp = app.config.wxappApi.api_regexp || WEAPP_REFERER; // 中间件需要识别的正则（字符串、表达式、数组）
    ...
    if (matched) {
      ...
      if (app.config.wxappApi.strict_mode) {
        ctx.body = {
          ret: 'ERROR',
    ...
    if (!app_token.token) {
      ...
      const key_id = `${app.config.wxappApi.token_prefix}:${app_token.token}`;
      const tokenData = await ctx.helper.kml_getToken(key_id);
